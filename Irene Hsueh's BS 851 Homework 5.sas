/******************** Irene Hsueh's BS 851 Homework 5 ********************/
proc format; 
...
data depression; 
	...
	label trt ="Treatment" agegrp="Age Group" remiss="Remission Status";
	format remiss remiss_fmt. death death_fmt.;
cards;
N Young 78.00 .00 1
N Young 80.00 .00 1
N Young 82.00 .00 1
...
proc print data=depression label;
run;
title;
...
title "Kaplan-Meier Survival Analysis of all Three Treatments";
...
title "Kaplan-Meier Survival Analysis of N vs A";
proc lifetest data=depression plots=(loglogs);
	where trt in ("N","A");
	time days*remiss(0);
	strata trt;
...
title "Kaplan-Meier Survival Analysis of N vs B";
proc lifetest data=depression plots=(loglogs);
	where trt in ("N","B");
	time days*remiss(0);
	strata trt;
...
title "Chi-Square Test for Proportion of Death";
